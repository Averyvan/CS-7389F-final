...
public class PacketReader {
	...
	private void config(String filename){
		...
		if (pcapReader == null) {
		  System.out.println("Error while opening file for capture: "+errbuf.toString());
	...
	public BasicPacketInfo nextPacket(){
		 ...
		 try{
		    int returncode = pcapReader.nextEx(hdr,buf);
			 if(returncode == Pcap.NEXT_EX_OK){
				 ...
				 //if (packetInfo == null){ // trying to see if removing this fixes error
				//	 packetInfo = getVPNInfo(packet);
				 //}					 
        
       //} else if (returncode == Pcap.NEXT_EX_EOF) { //can't do this... need the error to pass up to readPcapFile to stop looping safely...
         //System.out.println("all done with this file, closing");
         //pcapReader.close();
         //return null;
			 }else{
			   //System.out.println("packet throwing PcapClosedException because return value is " + returncode);
				 throw new PcapClosedException();
			 }
		 }catch(PcapClosedException e){
		  //System.out.println("packet PcapClosedException");
	...
	private BasicPacketInfo getIpv4Info(PcapPacket packet){
		BasicPacketInfo packetInfo = null;		
		// System.out.println("inside getIpv4Info"); // TRUE, does get here
		try {
						
			if (packet.hasHeader(ipv4)){
			  //System.out.println("packet has header ipv4");
          ...
          //System.out.println("packet has this Ip4 protocol: " + ip4Type.name() + ", description: " + ip4Type.getDescription());
					switch (ip4Type) {
						case IGMP:
					...
					//	System.out.println("getIpv4Info: {" + header.getName() + "} --description: {" + header.getDescription() + "} ");
					//}
				}
			} else { // no ipv4 header
			  //System.out.println("packet does not have ip4 header"); // ALWAYS TRUE
			}
		} catch (Exception e) {
		  //System.out.println("packet hit an exception trying to check if ip4 was in header");
			e.printStackTrace();
	...
	private BasicPacketInfo getIpv6Info(PcapPacket packet){
	  //System.out.println("packet has " + packet.getHeaderCount() + " headers");
		BasicPacketInfo packetInfo = null;
		try{
			if(packet.hasHeader(ipv6)){
			  //System.out.println("packet has ipv6 header!!");
				packetInfo = new BasicPacketInfo(this.generator);
			  ...
			  //System.out.println("packet has NO ipv6 header...");
			  //int headerCount = packet.getHeaderCount();
				//for(int i=0;i<headerCount;i++) {
				//	JHeader header = JHeaderPool.getDefault().getHeader(i);
				//	//JHeader hh = packet.getHeaderByIndex(i, header);
				//	System.out.println("getIpv6Info: {" + header.getName() + "} --description: {" + header.getDescription() + "} ");
				//}
				//if (headerCount == 0) {
				//  System.out.println("getIpv6Info: ZERO headers found in packet... ");
				//}
			}
		}catch(Exception e){
